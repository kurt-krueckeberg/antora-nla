= Drafted Official Reply to the Widow Krückeberg (4 Aug. 1824)
:page-role: wide

[.narrow]
== Summary

The is the *Draft (Concept) of the official reply (Rescript) to the widow
Krückeberg* It is the resolution to the petitioner: the prince has granted
her three years' service excemption from the *Burgrest labor days* and the
*forestry services*. 

The text of the Office Reply appears on the *top left side* of the page
(consisting of about three lines of text), and it continues on the *top
right side* of the page.

== Image

image::03-r.png[width=75%,align=left,title=Click to Enlarge,link=self]

[.narrow]
== Transliteration and Translation 

.Transliteration
[literal.bordered,subs="verbatim,quotes"]
....
Concept An die Witwe Krückeberg Nro. 10 in Bärenbusch.

Höchsten Orts ist der Witwe Krückeberg Nro. 10 in Bärenbusch auf dreÿ
Jahre Freÿheit von Burgrest- und Forstdeinst genädigst bewilligt
Bbg. 4. Aug. 1824.
F.p.

AKaas.
....

.Translation
[literal.bordered,subs="verbatim,quotes"]
....
Concept

To the widow Krückeberg No. 10 in Bärenbusch.

By the highest authority, the widow Krückeberg, No. 10 in Bärenbusch,
has been graciously granted exemption from Burgrest and forestry service
for three years.

Bückeburg, 4 August 1824.
For the record.
A. Kaas
....
