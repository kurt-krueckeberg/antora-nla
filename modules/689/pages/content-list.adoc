= Contents List
:page-role: wide

image::01-r.png[align=center,title="Click to Enlarge",link=self]

.Rescript Explained
****
A Rescript is abbreviated in the Acten Desingation, as " “Rescr.” or “KRes.” (Kammer-Rescript). in the Acten
Designation. It   is a *written official reply or order* issued by a higher authority in response to a petition,
report, or request.

In a legal context: it’s an *authoritative decision communicated in writing*.  In a financial-administrative
context, which is the case here, it’s a *formal directive, decree, or ruling.*

The best translation is "Official Order of the Chamber” or “Decree of the Chamber”.

*Etymology*:

In classical and medieval Latin, *rescriptum*  literally means “something written back” (re-scribere). In
Roman and later European law it *meant a written reply by the ruler* to a petition. German chancery practice
inherited the term directly from Latin, and it shows up in 18th–19th century files as Rescriptum or abbreviated
Rescr.  Rescript is simply the shortened form of the Latin Rescriptum, borrowed into German (Reskript) and then
into English.
****

== Transliteration 

[%header,cols="^1,9,6,2,2,2,^1",frame=none]
.Transliteration
|===
|No | Acten Designation (left) | Acten Designation (right) | Ort | Jahr | Mon | Z.

|1.
|xref:doc-01.adoc#doc-index-1-1[Vorstellung des Col. Krückeberg No 10 in Berenbusch------Anb.]
|Bewilligung eines Darlehens ad 200 r.
|Bbg.
|1841
|Octob.
^|2

|2.
|xref:doc-02.adoc#doc-index-2-1[Bericht Amts Bückeburg]
.2+.^|Cour. aus Cammer-Casses
|〃
|〃
|〃
^|7

|〃
|xref:doc-02.adoc#doc-index-2-2[Cpt. KRes. an dasselbe]
|〃
|〃
|〃
^|14

|3.
|xref:doc-03.adoc#doc-index-3-1[Vorst. des Colons Krückeberg]
.3+.^|gegen Verpfändung seiner sämmtlichen Ländereien
|Bbg.
|1841
|Nov.
^|11

|〃
|xref:doc-03.adoc#doc-index-3-2[Vortrag des GKR od Reck]
|〃
|〃
|〃
^|15

|〃
|xref:doc-03.adoc#doc-index-3-3[Cpt. KRes. an p Krückeberg]
|〃
|〃
|〃
|

|4.
|xref:doc-04.adoc#doc-index-4-1[Vorst. des p Krückeberg]
|
|Bbg.
|1841
|Nov.
^|21

|〃
|xref:doc-04.adoc#doc-index-4-2[Cpt. KRes. an Of. Creuzinger]
<|item Gesuch um Verkauf seiner Ländereien auf der Söhren-Weide betr.
|〃
|〃
|〃
^|25

|5.
|xref:doc-05.adoc#doc-index-5-1[Bericht des Of. Creuzinger Anb. Situations-Riß der Söhren-Weide]
|
|Bbg.
|1841
|Decbr.
^|10

|6.
|xref:doc-06.adoc#doc-index-6-1[Vortrag des GKR od Reck]
|
|〃
|〃
|〃
^|16

|〃
|xref:doc-06.adoc#doc-index-6-2[Höchstes Rescr. Ser. Reg.]
|
|〃
|〃
|〃
^|19

|〃
|xref:doc-06.adoc#doc-index-6-3[P. V. des H od Reck u. Höchstes Rescr. Sr. Durchlaucht]
|
|〃
|〃
|〃
^|23

|7.
|xref:doc-07.adoc#doc-index-7-1[Cpt. Rescr. aus [sic: should be *an*\] Amt Bückeburg]
|
|Bbg.
|1841
|Decbr.
^|24

|8.
|xref:doc-08.adoc#doc-index-8-1[Bericht Amts Bückeburg]
|
|Bbg.
|1842
|Jan.
^|2

|〃
|xref:doc-08.adoc#doc-index-8-2[Cpt. KRes. an dasselbe]
|
|〃
|〃
|〃
^|20

|〃
|xref:doc-08.adoc#doc-index-8-3[desgl[eichen\]. an den K. R. Berger]
|
|〃
|〃
|〃
|

|9.
|xref:doc-09.adoc[Bericht Amts Bückeburg]
|
|Bbg.
|1842
|Febr.
^|8

|10.
|Obligation über aus Fürstl. Kammerkasse geliehene 200 r. Courant
|Obligation.
|
|
|
|
|===

.Carifications
****
[NOTE]
====
*#10* in the case file is *not a memo*; it’s the *Obligation (the sealed promissory note itself)*. Originals of
*promissory notes* were often *kept in a separate “obligations” series* and, after repayment, *returned to the
debtor*—so they commonly *don’t carry* the file’s boxed corner number and may not be physically in the case file
anymore.  _That’s why #10 has no boxed number in the case file._
====

[NOTE]
====
*Row 7 (“aus” [sic])* — The index writes *Cpt. K. Res. aus Amt Bückeburg*, but the document itself is headed
*Concept an das Amt Bückeburg*, bears no *pr. in Cam.* (incoming-presented) mark, and carries the Kammer closing
(*Fürstl: p.*). We therefore keep **aus** in the transliteration (diplomatic accuracy) and render the English **to
the Amt Bückeburg** (sense), noting the likely clerical slip.
====
****

== English Version

[%header,cols="^1,9,6,2,2,2,^1",frame=none]
.Translation (corrected)
|===
|Item No | Document Decription (left) | Subject note (right) | Place | Year | Mon | Z.

|1.
|Petition of Colon Krückeberg No. 10 in Berenbusch, with enclosure [to No. 2]
|Approval of a loan of 200 r.
|Bbg.
|1841
|Oct.
^|2

|2.
|Report of the Office of Bückeburg
.2+.^|Funds from the Chamber Treasury
|〃
|〃
|〃
^|7

|〃
|Official Order of the Chamber to the same
|〃
|〃
|〃
^|14

|3.
|Petition of Colon Krückeberg
.3+.^|on condition of pledging all of his lands [as collateral].
|Bbg.
|1841
|Nov.
^|11

|〃
|Report of the Privy Chamber Councillor Reck
|〃
|〃
|〃
^|15

|〃
|Official Order of the Chamber to Colon Krückeberg
|〃
|〃
|〃
|

|4.
|Petition of Colon Krückeberg
|
|Bbg.
|1841
|Nov.
^|21

|〃
|Official Order of the Chamber to Officer Creuzinger
<|Further petition concerning the sale of his lands on the Söhren pasture
|〃
|〃
|〃
^|25

|5.
|Report of Officer Creuzinger with attached site plan of the Söhren pasture
|
|Bbg.
|1841
|Dec.
^|10

|6.
|Report of Privy Chamber Councillor Reck
|
|〃
|〃
|〃
^|16

|〃
|Highest Official Order of His Serene Highness
|
|〃
|〃
|〃
^|19

|〃
|Protocol by Councillor Reck and Highest Official Order of His Serene Highness
|
|〃
|〃
|〃
^|23

|7.
|Official Order of the Chamber to the Office of Bückeburg
|
|Bbg.
|1841
|Dec.
^|24

|8.
|Report of the Office of Bückeburg
|
|Bbg.
|1842
|Jan.
^|2

|〃
|Official Order of the Chamber to the same
|
|〃
|〃
|〃
^|20

|〃
|likewise to Privy Councillor Berger
|
|〃
|〃
|〃
|

|9.
|Report of the Office of Bückeburg
|
|Bbg.
|1842
|Feb.
^|8

|10.
|Bond for 200 r. courant borrowed from the Princely Chamber Treasury
|Obligation.
|
|
|
|
|===

=== Comments and Insights

The first column *"No"* is a case item or grouping number for a cluster of related documents. Each number groups together
the related petitions, reports, and official orders concerning a particular stage in the case, in which one aspect
of the overall decision making process is under consideration. The description of the overall case, what the Rentcammer is
trying to ultimately decided, is given by entry 1: the petition of Colon Krückeberg of No. 10 Berenbusch for a
loan of 200 Reichsthaler.

In entry 3's *"Acten Designation (right)"* "gegen" in "gegen Verpfändung seiner sämmtlichen Ländereien" does not mean
"against", its typically meaning, but rather "on condition of". The Rentcammer was willing to consider a loan but only
against the pledge of his entire holding. The Chamber’s willingness to approve a loan was conditional upon his pledging
all of his landholdings as collateral. 

=== Explanation

==== Column keys
No:: Running item number (1–10) within this case file’s index.
Acten Designation (left):: Short description of the piece (who/what).
Acten Designation (right):: Filing rubric / topic or subject note.
Ort:: Place (e.g., *Bbg.* = Bückeburg).
Jahr / Mon:: Year / Month.
Z.:: Monthly registry sequence number (Zahl) assigned in the office register.

==== Row-by-row
1:: Petition by Colon Krückeberg (No. 10, Berenbusch) with enclosure; approval of a 200-r. loan.
2:: Report from the Amt Bückeburg; right rubric **Corr. aus** = “Correspondence, outgoing”.
2 (〃):: Kammer-Resolution to the same office; right rubric **Cammer-Casse** (treasury); Z. 14.
3:: Petition of Krückeberg; right (rowspan): “gegen Verpfändung seiner sämmtlichen Ländereien” (re: pledging all his lands as collateral); Z. 11.
3 (〃):: Memorandum/submission by **GKR od Reck**; Z. 15.
3 (〃):: Kammer-Resolution to **p Krückeberg** (*p.* = formulaic “pro/persona”); Z. — (not recorded in the index).
4:: Another petition from Krückeberg; right column blank; Z. 21.
4 (〃):: Kammer-Resolution to Oberförster Creuzinger; “item **Gesuch um** …” indicates a related petition under the same matter; Z. 25.
5:: Report of Oberförster Creuzinger with enclosure (site sketch) of the **Söhren-Weide**; right (rowspan): “Verkauf seiner Ländereien auf der Söhren-Weide betr.” (re: sale of his parcels there); Z. 10.
6:: Memorandum by **GKR od Reck** (cf. above); Z. 16.
6 (〃):: **Höchstes Rescr.** (sovereign rescript/order); Z. 19.
6 (〃):: **P. V.** note by the councillor plus another Highest Rescript of His Serene Highness; Z. 23.
7:: Rescript received from Amt Bückeburg; Z. 24.
8:: Report from Amt Bückeburg (Jan. 1842); Z. 2.
8 (〃):: Kammer-Resolution to the same office; Z. 20.
8 (〃):: Likewise to **K. R. Berger** (Kammer-/Kameralrat); Z. —.
9:: Further report from Amt Bückeburg (Feb. 1842); Z. 8.
10:: Promissory Note (*Obligation*) for the 200 r. Courant loan drawn from the princely Kammerkasse.

==== Abbreviation keys (from the original German index)

Vorst.:: Vorstellung (petition)
Gg. / Gg. K. Res.:: Gegen / Gegen-Kammer-Resolution (issued to / chamber resolution in reply)
Rescr.:: Rescript (formal written order/resolution)
Of.:: Oberförster (Chief Forester)
Cammer-Casse / Kammerkasse:: Chamber Treasury
desgl.:: desgleichen (likewise)
Corr. aus:: Correspondence, outgoing (registry rubric)
P. V.:: Filing note such as *Protokoll-Vermerk* / *per Votum* (left as written)
Colon:: Holder of a full farm (estate tenant) in the local agrarian order
rt / r. / Courant:: Reichstaler (currency); *courant* = current circulating money
Amt (Bückeburg):: District/administrative office at Bückeburg
Register no. (Zahl):: Monthly registry sequence number assigned in the office register

