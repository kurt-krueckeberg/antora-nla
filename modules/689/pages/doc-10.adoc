= #10 Document: Follow-ups to Promissory Note for 200 Reichsthaler in currency, borrowed from the princely treasury. 
:page-role: wide
...
[cols="1a,6a",width="60%",frame=ends,grid=rows]
|===
2+|*Docketing marks*
...
[.widepair,cols="1a,1a",grid=none,frame=none]
|===
...
[literal.bordered,subs="verbatim,quotes"]
....
Copy. Order of the Chamber for the chamber treasurer Berger + dated
Bückeburg, 21 April 1845
...
[.widepair,cols="1a,1a",options="header",frame=none,grid=none]
|===
...
....
Approved.  
Entered. The accrued interest from **12 Jan to 28 Apr 1845 (3½
months)**, paid — **[initialed: DH]** — **2 thalers 12
groschen**.  
Poppelbaum.

Notes:  
* **DH** are large approval initials (a signature-like mark), not running text.  
* **rt** = *Reichsthaler*, **gr** = *Groschen*.
....
|===

== Concept Quittung

[cols="1a,6a",frame=none,grid=none,width="60%"]
|===
|Transliteration | English translation & notes

|ohlt
|*Meaning:* Clerk’s initials used as a routing/attestation tick; not running text.\
*Placement on page:* Immediately beneath the NC number at the very top margin.\
*Who / when:* Entered by the registry/Rentkammer scribe associated with the action dated **5 May 1845** on this page.

|exp
|*Meaning:* Abbrev. of *expediert/Expedition* → “dispatched/forwarded”; a routing/handling 
*Placement on page:* In the left margin near **“Concept / Quittung.”** (on this sheet); elsewhere in the file it’s paired with a name/date.\
*Who / when:* Written by the expedition/dispatch clerk (in this case linked to **Schöttd.** with the date **8/5** on the related entry, i.e., **8 May 1845**).
...
Since Colon (tenant farmer) Krückeberg has repaid to the Chamber
cash office, by crediting/set-off (*Zurechnung*), the principal
recorded in the foregoing obligation of 12 January 1842—amounting
to **200 Reichsthaler Courant**—together with arrears and current
interest, **he is hereby receipted for it** and **the
cancellation of the mortgage is approved**.  
Bburg (= Bückeburg), 5 May 1845.  
(L.S.) = *locus sigilli* (“place of the seal”).  
F. p. (kept as-is; standard signature formula in these files).  
